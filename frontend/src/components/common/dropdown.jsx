...
const Dropdown = ({ options, onChange, label, ...props }) => {
  ...
  return (
    <div id={wrapperId} class={`${props.wrapperclasses || ''} w-full relative`}>
      ...
      <input
        id={id}
        class={twMerge(`w-full shadow-ds2 pr-4 py-2.5 lg:py-3.5 pl-4 text-sm font-semibold bg-transparent border-2 text-gray-800 border-gray-400 focus:border-gray-900 focus:ring-0 focus:ring-offset-0 rounded-md cursor-pointer transition-all duration-300`, props.className)}
      ...
      {selectedOption && (
        <div class={`w-fit h-full absolute right-3 ${label ? 'top-4' : 'top-0'} flex justify-center items-center ${props.className.includes('hidden') || props.className.includes('opacity-0') ? 'hidden opacity-0' : ''}`}>
